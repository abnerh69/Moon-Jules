...
def test_una_sesion_fallida_se_reactiva():
    """Verificado contra el API el 2026-08-26: `sendMessage` sobre una
    sesión FAILED devuelve 200 y la sesión vuelve a IN_PROGRESS.

    Durante veinte entregas se asumió lo contrario. El Spike 01 dejó la
    pregunta abierta y la suposición prudente se repitió como si fuera
    un hecho, dejando fuera nueve de cada diez sesiones problemáticas.
    Reactivarlas es la razón de ser de este proyecto.
    """
...
def test_en_read_only_una_fallida_solo_alerta():
    s = sess(SessionState.FAILED).with_failure("algo")
    r = assess(s, Freshness(), NOW, mode=AutonomyMode.READ_ONLY)
    assert r.action is Action.ALERT
    assert r.downgraded_from is Action.NUDGE


def test_una_fallida_que_murio_preguntando_se_distingue():
    """Diagnósticamente no es lo mismo que fallar por dependencias, y
    hasta ahora las nueve fallidas se veían idénticas."""
    s = sess(SessionState.FAILED).with_details(
        reason="Jules was unable to complete the task.", died_asking=True
    )
    r = assess(s, Freshness(), NOW, mode=AutonomyMode.UNBLOCK_ONLY)
    assert r.verdict is Verdict.FAILED_ASKING
    assert r.action is Action.NUDGE, "muerta es peor que mal contestada"
    assert "pregunta" in r.reason


def test_una_fallida_recien_reactivada_no_se_toca_otra_vez():
    """Entre el intento y su plazo de verificación no se insiste."""
    s = sess(SessionState.FAILED).with_failure("algo")
    n = NudgeRecord(sent_at=ago(minutes=2))
    r = assess(s, Freshness(), NOW, mode=AutonomyMode.UNBLOCK_ONLY, nudge=n)
    assert r.action is Action.NONE
    assert "reactivandose" in r.reason


def test_una_fallida_que_no_revivio_escala_a_alerta():
    """Sigue en FAILED pasado el plazo: el prompt no la levantó."""
    s = sess(SessionState.FAILED).with_failure("algo")
...
def test_una_fallida_no_se_reintenta_sin_fin():
    s = sess(SessionState.FAILED).with_failure("algo")
    n = NudgeRecord(sent_at=ago(hours=2), count=P.max_nudges_per_session)
    r = assess(s, Freshness(), NOW, mode=AutonomyMode.UNBLOCK_ONLY, nudge=n)
...
# --------------------------------------------------------------------
# la regla del abandono
# --------------------------------------------------------------------


def test_una_sesion_abandonada_no_se_reactiva_sola():
    """«Nunca actuar sobre una sesión que nadie ha mirado en 48 horas.»

    Reactivar trabajo de hace semanas gasta cuota y abre PRs que nadie
    pidió. La decisión de recuperarlo es del arquitecto, no de una
    herramienta que vigila.
    """
    s = sess(SessionState.FAILED).with_failure("algo de mayo")
    vieja = Freshness(ago(days=100), "sessionFailed", last_touch_at=ago(days=100))
    r = assess(s, vieja, NOW, mode=AutonomyMode.UNBLOCK_ONLY)
    assert r.action is Action.ALERT
    assert r.downgraded_from is Action.NUDGE
    assert "no se actua sola" in r.reason


def test_una_fallida_de_hoy_si_se_reactiva():
    s = sess(SessionState.FAILED).with_failure("algo de hoy")
    r = assess(
        s,
        Freshness(ago(hours=3), "sessionFailed", last_touch_at=ago(hours=3)),
        NOW,
        mode=AutonomyMode.UNBLOCK_ONLY,
    )
    assert r.action is Action.NUDGE
...
def test_haberla_mirado_la_rescata_del_abandono():
    """El caso que justifica medir el contacto de *cualquiera*: una
    sesión abierta hace meses a la que el arquitecto escribió ayer no
    está abandonada."""
    s = sess(SessionState.FAILED).with_failure("vieja pero atendida")
    fresca = Freshness(ago(days=100), "sessionFailed", last_touch_at=ago(hours=6))
    r = assess(s, fresca, NOW, mode=AutonomyMode.UNBLOCK_ONLY)
    assert r.action is Action.NUDGE


def test_el_plazo_de_abandono_es_configurable():
    s = sess(SessionState.FAILED).with_failure("algo")
    hace_tres_dias = Freshness(ago(days=3), "sessionFailed", last_touch_at=ago(days=3))
    corta = Policy(abandon_after_h=24)
    larga = Policy(abandon_after_h=24 * 30)
    assert assess(s, hace_tres_dias, NOW, policy=corta,
                  mode=AutonomyMode.UNBLOCK_ONLY).action is Action.ALERT
    assert assess(s, hace_tres_dias, NOW, policy=larga,
                  mode=AutonomyMode.UNBLOCK_ONLY).action is Action.NUDGE


def test_una_estancada_vieja_tampoco_se_toca():
    """La regla no es solo para fallidas: frena toda escritura."""
    vieja = Freshness(ago(days=60), "progressUpdated", last_touch_at=ago(days=60))
    r = assess(sess(SessionState.IN_PROGRESS), vieja, NOW,
               mode=AutonomyMode.UNBLOCK_ONLY)
    assert r.verdict is Verdict.STALLED
    assert r.action is Action.ALERT


def test_alertar_sobre_una_abandonada_sigue_estando_bien():
    """La regla frena las escrituras, no la información: enterarse de
    que algo está mal nunca sobra."""
    vieja = Freshness(ago(days=60), "progressUpdated", last_touch_at=ago(days=60))
    r = assess(sess(SessionState.IN_PROGRESS), vieja, NOW,
               mode=AutonomyMode.UNBLOCK_ONLY)
...
def test_sin_saber_cuando_se_toco_no_se_bloquea():
    """Una sesión recién creada aún no tiene actividades. Bloquearla
    sería tratar «no lo sé» como «es vieja»."""
    r = assess(sess(SessionState.QUEUED), Freshness(), NOW,
               mode=AutonomyMode.UNBLOCK_ONLY)
    assert r.action is not Action.ALERT or r.downgraded_from is not Action.NUDGE
